...
import { readResourceDetails } from "./resource-details";
...
const palette = mountPalette(renderer, items, { theme, history: loadHistory(), currentWorkspaceId: parseLaunchContext()?.workspaceId, run: async (item, input) => {
  ...
  return result;
}, close: () => renderer.destroy(), update: installUpdate, dismissUpdate, sessionJob: runSessionJob, panePreview: readPanePreview, resourceDetails: readResourceDetails });
